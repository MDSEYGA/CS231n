...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        x_vector = x[:, i, :].reshape(N, D)
        next_h, cache[i] = rnn_step_forward(x_vector, prev_h, Wx, Wh, b)
        h[:, i, :] = next_h
        prev_h = next_h
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dnext_h = dh[:, i, :].reshape(N, H) + dprev_h
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, cache[i])
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dx, dWx = da @ Wx.T, x.T @ da
    dprev_h, dWh = da @ Wh.T, prev_h.T @ da
    db = da.sum(axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        x_vector = x[:, i, :].reshape(N, D)
        next_h, next_c, cache[i] = lstm_step_forward(x_vector, prev_h, prev_c, Wx, Wh, b)
        h[:, i, :] = next_h
        prev_h = next_h
        prev_c = next_c
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    x, Wx, Wh = cache[0][0], cache[0][3], cache[0][4]
    N, D = x.shape
