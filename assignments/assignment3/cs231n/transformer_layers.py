...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[0, :, 0::2] = torch.sin(positions * pows)
        pe[0, :, 1::2] = torch.cos(positions * pows)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        scores = torch.matmul(qi, ki.transpose(2, 3)) / math.sqrt(E // H)
        ...
        def merge_heads(x):
            N, H, S, head_dim = x.shape
            x = x.transpose(1, 2)
            x = x.reshape(N, S, H * head_dim)
            return x
        
        yi = torch.matmul(weights, vi)
